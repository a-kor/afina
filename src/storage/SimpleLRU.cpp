...
void SimpleLRU::raise(const lru_index_t::iterator it) {
    _lru_list.push_back(std::move(*it->second));
    _lru_list.erase(it->second);
    auto raised_it = --_lru_list.end();
    _lru_index[(*raised_it)->key] = raised_it;
}
...
bool SimpleLRU::Put(const std::string &key, const std::string &value) {
    auto lookup_key = key;
    auto it = _lru_index.find(std::ref(lookup_key));
