...
private:
    ...
    using lru_index_t = std::map<std::reference_wrapper<const std::string>, lru_list_t::iterator, std::less<std::string>>;
    
	// Makes *lru_index_it the most recently used.
    void make_mru(const lru_index_t::iterator lru_index_it);
